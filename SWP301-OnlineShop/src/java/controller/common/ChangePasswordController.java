...
public class ChangePasswordController extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
            throws ServletException, IOException {
        String tokenSend = "", tokenSave = "", email = "";
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        email = request.getParameter("hdEmail");
        newPassword = request.getParameter("txtPassword1").trim();
        user = userDb.getUserByEmail(email);
        if (user != null && newPassword != null && newPassword != "") {
            user.setPassword(newPassword);
            result = userDb.updateUser(user);
        }
        if (result) {
            request.setAttribute("messTrue", "Password change successful!");
        } else {
            request.setAttribute("messFalse", "Password change failed!");
        }
